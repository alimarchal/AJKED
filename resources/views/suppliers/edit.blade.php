    ...
    <form method="post" action="{{route('supplier.update',$supplier->id)}}">
        ...
        <div class="form-row">
            ...
            <div class="form-group col-md-4">
                ...
                <input type="text" name="name_of_supplier_firm" value="{{$supplier->name_of_supplier_firm}}" id="name_of_supplier_firm" class="form-control">
            </div>

            <div class="form-group col-md-3">
                <label for="ajked_registration_no">AJKED Registration Number</label>
                <input type="text" name="ajked_registration_no" id="ajked_registration_no" value="{{$supplier->ajked_registration_no}}"  class="form-control">
            </div>


            <div class="form-group col-md-5">
                <label for="product_id">Name of Item</label>
                @php $item_list = $supplier->supplier_items->pluck('product_id')->toArray();  @endphp
                <select class="form-control select2" id="product_id" name="product_id[]" multiple required>
                    <option value="">None</option>
                    @foreach(\App\Models\Product::all() as $item)
                        <option value="{{$item->id}}"
                            @if(in_array($item->id,$item_list))
                                selected
                            @endif
                        >{{$item->name}}</option>
                    @endforeach
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="status" name="status" required>
                    ...
                    <option value="NonActive" @if($supplier->status == "Blacklisted") selected @endif>Blacklisted</option>
                </select>
            </div>
